...
public class GeneratorLogic(IPaimon _canningService, IAttributeDeluxe _attributeservice)
{
    public string GenerateConstructor(ITypeSymbol type, string nameOfFieldWithAttr)
    {
        ...
        {
            case null:
                if (locationParams.strs is null)
                {
                    throw new ArgumentNullException(nameof(locationParams.strs) + "was null. This situation is incorrected");
                }
    ...
    public (int[]? nums, string[] strs) GetValueWithAtribute(ITypeSymbol type, string nameOfMethodWithAttribute)
    {
        ...
        var attr = field.GetAttributes().ToList().Where(x => _attributeservice.GetName(x) == nameof(UseableParamsAttribute)).SingleOrDefault() ?? throw new Exception("Logic exception.");
    ...
    {
        var constructor = type.GetMembers().OfType<IMethodSymbol>().Where(x => x.IsConstructor()).SingleOrDefault() ?? throw new NullReferenceException("Constructor wasn`t found"); // костыль не адаптивный :(

        IList<IParameterSymbol> parameters = new List<IParameterSymbol>();

        foreach (int? i in indexesArg)
        {
            if (i > constructor.Parameters.Length || i is null)
            {
                throw new IndexOutOfRangeException("Param wasn`t found in nums");
            }
    ...
    public IList<IParameterSymbol> GetConstructorAndParams(ITypeSymbol type, List<string> NamesArg)
    {
        ...
        var constructor = n.OfType<IMethodSymbol>().Where(x => x.IsConstructor()).SingleOrDefault() ?? throw new NullReferenceException("Constructor wasn`t found "); // костыль не адаптивный :(

        IList<IParameterSymbol> parameters = new List<IParameterSymbol>();

        foreach (string name in NamesArg)
        {
            var param = constructor.Parameters.Where(x => x.Name == name).SingleOrDefault() ?? throw new NullReferenceException("Param wasn`t found in strs");
